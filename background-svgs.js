...
class BackgroundSVG {
    ...
    addClickHandler() {
        // Add single click handler to document
        document.addEventListener('click', () => {
            // Morph all SVGs
            this.svgs.forEach(svg => {
                this.morphToRandomShape(svg);
            });
        });
    }
...
class TitleSVG {
    ...
    addClickHandler() {
        this.container.addEventListener('click', () => {
            this.letterSvgs.forEach(path => {
                // Only morph to vine or snail
                const targetSvgs = svgCollection.filter(s => s.name === 'vine' || s.name === 'snail');
                const targetSvg = targetSvgs[Math.floor(Math.random() * targetSvgs.length)];
                this.morphPath(path, targetSvg);
            });
        });
    }
